...
LRESULT OnLButtonDown( HWND hWnd, WPARAM wParam, LPARAM lParam, LPCLASSDATA lpcd )
{
	...
	/*
	 *	Check if we are on a
	 *	hyperlink.
	 */
	if ( Parser->bParseHyperLinks && RunHyperlink( lpcd ))
		return 0;
...
{
	return;   // Tooltip function disabled!
}
...
LRESULT OnMouseMove( HWND hWnd, WPARAM wParam, LPARAM lParam, LPCLASSDATA lpcd )
{
	 ...
	 */
	if ( Parser->bParseHyperLinks && ! lpcd->bTipShowing && ! lpcd->bHoverTracking && lpcd->dwPrevPos != lParam )
	{
		/*
...
LRESULT OnMouseWheel(  HWND hWnd, WPARAM wParam, LPARAM lParam, LPCLASSDATA lpcd )
{
	 ...
	 */
	if ( abs( lpcd->nWheelDelta ) >= WHEEL_DELTA )
	{
		/*
