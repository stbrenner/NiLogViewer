 ...
 *	    All Rights Reserved.
 *
 *	Parts of this code (C) Copyright  Andrei Stcherbatchenko.
 *
		...
		if ( pcUnicodeBuf )
		{
			...
			if ( ReadFile( hFile, pcUnicodeBuf, dwSize * sizeof( WCHAR ), pdwBytesRead, lpOverlapped ))
			{
				...
				*( pdwBytesRead) /= sizeof( WCHAR );
				 ...
				 */
				if (( *pdwBytesRead = ( DWORD )WideCharToMultiByte( CP_ACP, 0, pcUnicodeBuf, ( int )( *pdwBytesRead ), pcReadBuf, ( int )( *pdwBytesRead ), NULL, NULL ))) 
				{
					/*
...
BOOL LoadFile( LPCLASSDATA lpcd, LPCTSTR lpszFileName )
{
	...
	DWORD		dwBytesRead, dwBufferIndex = 0;
	SHORT		sCookie;
	...
	while ( 1 )
	{
		 ...
		 */
		if ( ! ReadFile( hFile, &sCookie, sizeof( SHORT ), &dwBytesRead, NULL ))
		{
			bError = TRUE;
			break;
		}
		 ...
		 */
		if ( ! ( sCookie == UNICOOKIE ? ReadUnicodeFile : ReadFile)( hFile, pcReadBuf, READ_BUF_SIZE, &dwBytesRead, NULL ))
		{
			bError = TRUE;
			break;
		}
    ...
    // We assume that lines are separated by \n, so file mode is UNIX
...
BOOL __stdcall WriteUnicodeFile( HANDLE hFile, LPVOID pcWriteBuf, DWORD dwSize, LPDWORD pdwBytesWritten, LPOVERLAPPED lpOverlapped )
{
	...
	else
	{
		...
		if ( pcUnicodeBuf )
		{
			 ...
			 */
			DWORD dwConverted;
			if (( dwConverted = MultiByteToWideChar( CP_ACP, 0, pcWriteBuf, dwSize, pcUnicodeBuf, dwSize )))
			{
				/*
...
BOOL SaveFile( LPCLASSDATA lpcd, LPCTSTR lpszFileName )
{
	...
	while ( 1 )
	{
		...
		if ( lpcd->bUnicode )
		{
			SHORT sCookie = UNICOOKIE;
			if ( ! WriteFile( hFile, &sCookie, sizeof( SHORT ), &dwBytesWritten, NULL ))
			{
				bError = TRUE;
				break;
			}
			if (( int )dwBytesWritten != sizeof( SHORT))
			{
				bError = TRUE;
				break;
			}
