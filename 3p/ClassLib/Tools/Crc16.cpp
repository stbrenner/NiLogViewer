...
ClsCrc16::ClsCrc16( WORD wPolynomial )
{
...
void ClsCrc16::CrcInitialize( WORD wPolynomial /* = 0x1021 */ )
{
	...
	if ( wPolynomial != 0x1021 )
	{
		...
		if ( m_pCrcAllocated == NULL ) m_pCrcAllocated = new WORD[ 256 ];
		...
		for ( WORD i = 0; i <= 255; i++ )
		{
			...
			for ( int j = 8; j > 0; j-- )
			{
				if( crc & 0x8000 ) crc = ( WORD )(( crc << 1 ) ^ wPolynomial );
				else		   crc <<= 1;
			}
			m_pCrcAllocated[ i ] = ( WORD )( crc & 0xFFFF );
		}
...
WORD ClsCrc16::CrcAdd( LPBYTE pData, DWORD dwLength )
{
	...
	for ( DWORD i = 0; i < dwLength; i++ )
		m_sCRC = ( WORD )(( m_pCrcTable[ (( m_sCRC ) ^ ( *pData++ )) & 0xFF ] ^ (( m_sCRC ) >> 8 )));

	return ( WORD )( m_sCRC ^ 0xFFFF );
}

// Compute CRC for this block of data.
WORD ClsCrc16::Crc( LPBYTE pData, DWORD dwLength )
{
	// Must be a valid pointer.
	if ( pData == 0L ) 
		return 0;
