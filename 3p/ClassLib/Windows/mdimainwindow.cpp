...
ClsMenu *ClsMDIMainWindow::MDISetMenu( ClsMenu *pFrameMenu, ClsMenu *pWindowMenu )
{
	...
	// Set the new menu.
	HMENU hMenu = ( HMENU )m_Client.SendMessage( WM_MDISETMENU, ( WPARAM )( HMENU )( pFrameMenu  ? pFrameMenu->GetHMenu()  : NULL ), 
								    ( LPARAM )( HMENU )( pWindowMenu ? pWindowMenu->GetHMenu() : NULL ));
	// Wrap the old menu.
	if ( hMenu )
